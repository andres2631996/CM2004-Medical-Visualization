...
class SliceRender:
    def __init__(self, filename, frame, orie='sagittal'):
        ...
        (xMin, xMax, yMin, yMax, zMin, zMax) = reader.GetExecutive().GetWholeExtent(
    ...
    def MouseMoveCallback(self, obj, event):
        ...
        if self.actions["Slicing"] == 1:
            ...
            current_window = self.interactor.GetRenderWindow()
...
if __name__ == "__main__":
    ...
    filename = 'labs/data/head_self_t1.nii.gz' 
    sliceRen = SliceRender(filename, orie='axial')

    window = vtk.vtkRenderWindow()
    window.AddRenderer(sliceRen.renderer)
    window.SetInteractor(sliceRen.interactor)
    window.Render()

    # Start interaction
    sliceRen.interactor.Start()
